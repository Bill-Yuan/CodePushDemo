...
export default class App extends Component<Props> {
	...
	_deploymentKey = () => {
	   return (Platform.OS === 'ios' ?
           '1eDZyB0aUo5mj1v4toXNZf-5BEAk193b9b68-ab9c-43f6-abff-ef50b9329e57' :
           'fHxHc8pq2qPa7_QggE3U6sYUG4xj193b9b68-ab9c-43f6-abff-ef50b9329e57')
    }
			...
			codePush.checkForUpdate(
				...
				.then((remotePackage) => {
					if(remotePackage) {
						...
						console.log(remotePackage)
						Alert.alert('温馨提示','检测到更新版本')
						this.setState({remotePackage, hasCheckUpdate: true})
					} else {
						this.setState({hasCheckUpdate: true})
						Alert.alert('温馨提示','当前已是最新版本')
					}
					checkComplete()
				})
				.catch((error) => {
					console.log(`check update get error`+ JSON.stringify(error));
					checkComplete()
				})
		}
	})()
	...
	_downLoadFromRemote = (() => {
		...
		return () => {
			...
			this.state.remotePackage.download(this._downloadProgressCallback)
				...
				.catch((error) => {
					Alert.alert('温馨提示', JSON.stringify(error))
					downloadComplete()
				})
		}
	})()
